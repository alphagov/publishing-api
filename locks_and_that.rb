...
# TODO: for all of the digests past TTL, get matches from the following array
# (might be worth filtering our entries with no lock_digest). Filter our
# authenticated_user from the args (seems to be an array of hashes with the
# second - if present - containing the authenticated_user)
Sidekiq::RetrySet.new.to_a.map do
  it.item.slice(*%w[
    queue
    class
    args

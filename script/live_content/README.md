...
```sh
./script/live_content/diff_frontend_v2 base-path-of-document
```
...
```sh
./script/live_content/bulk_diff_frontend_v2
```
...
We originally wrote the frontend diffing scripts in Bash, but have since ported
them to Ruby following the same conventions as other GraphQL testing scripts
(including the ones for diffing content items). The original scripts were more
flexible (they could be run against non-development environments, for instance),
so they're worth checking out if you have a non-standard use case.

```sh
./script/live_content/diff_frontend_v1
```

```sh
./script/live_content/bulk_diff_frontend_v1 --base-paths-file-path path/to/file --environment p
```

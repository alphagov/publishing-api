...
RSpec.describe WellFormedContentTypesValidator do
  let(:record) { double(:record, errors: ActiveModel::Errors.new(nil)) }
  ...
  it "rejects values where the content key is missing" do
    ...
    # this needs tidying, but you get the idea:
    expect(record.errors.find { |e| e.attribute == :some_attribute }.type).to eq("the 'text/html' content type does not contain content")
    # then do the same for the ones below...

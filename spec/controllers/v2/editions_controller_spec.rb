  ...
  describe "index" do
    ...
    context "filtered by document type" do
      it "returns only the document type specified" do
        create(:draft_edition, document_type: "taxon", id: 10_000)
    ...
    context "filtered by CMS entity ids" do
      it "returns only editions that match" do
        ...
        get :index, params: { cms_entity_ids: %w[id:1 id:2] }
        expect(parsed_response["results"].count).to eq(20)
        expect(parsed_response["links"]).to eq([
          { "href" => "http://test.host/v2/editions?cms_entity_ids%5B%5D=id%3A1&cms_entity_ids%5B%5D=id%3A2", "rel" => "self" },
        ])
      end
    end

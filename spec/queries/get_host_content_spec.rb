    ...
    context "sorting" do
      ...
      def expect_sort_call_to_have_been_made_for(order_field:, order_direction:)
        expect(ActiveRecord::Base.connection).to have_received(:select_all) { |arel_query|
          ordering_nodes = arel_query.orders
          expect(ordering_nodes).to be_one

          if order_direction == :asc
            expect(ordering_nodes.first).to be_a(Arel::Nodes::Ascending)
            expect(ordering_nodes.first.expr).to eq(order_field)
          else
            expect(ordering_nodes.first).to be_a(Arel::Nodes::NullsLast)
            expect(ordering_nodes.first.expr).to be_a(Arel::Nodes::Descending)
            expect(ordering_nodes.first.expr.expr).to eq(order_field)
          end
        }
      end
    end

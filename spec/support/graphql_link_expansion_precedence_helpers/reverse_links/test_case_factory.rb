      ...
      class << self
        def all(source_content_ids_differ:)
          ...
          query_values
            ...
            .map { { **_1, source_editions: _2 } }
            .reject { |test_case|
              [
                redundant_locale(test_case),
                mismatching_link_type(test_case),
                *unless source_content_ids_differ
                   [
            ...
            .map { TestCase.new(**it, source_content_ids_differ:) }
        end
      ...
      private
        ...
        def redundant_locale(test_case)
          test_case[:root_locale] == Edition::DEFAULT_LOCALE &&
            test_case[:source_editions].any? { it[:locale] == "hu" }
        end
        ...
        def mismatching_link_type(test_case)
          test_case[:source_editions]
            .map { it[:permitted_unpublished_link_type] }
            .compact.uniq.size > 1
        end

        # given the source content ID is the same, the document must be the same
        ...
        def document_with_two_editions_in_live_content_store(test_case)
          fields_equal(*test_case[:source_editions], :locale) &&
            test_case[:source_editions]
              .map { it[:state] }
              .sort == %w[published unpublished]
        end

        # given the source content ID is the same, the edition must be the same
        ...
        def duplicate_edition_and_link_kind(test_case)
          fields_equal(*test_case[:source_editions], :link_kind, :state, :locale)
        end
        ...
        def single_edition_with_varying_properties(test_case)
          fields_equal(*test_case[:source_editions], :state, :locale) &&
            [
              !fields_equal(*test_case[:source_editions], :renderable_document_type),
              test_case[:source_editions]

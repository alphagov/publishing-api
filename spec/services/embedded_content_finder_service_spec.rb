RSpec.shared_examples "finds references" do |document_type|
  describe "when content is a #{document_type}" do
    ...
    it "finds content references" do
      body = "{{embed:#{document_type}:#{embedded_content_reference_1.friendly_id}}} {{embed:#{document_type}:#{embedded_content_reference_2.friendly_id}}}"

      links = EmbeddedContentFinderService.new.fetch_linked_friendly_ids(body, Edition::DEFAULT_LOCALE)
    ...
    it "errors when given a content ID that is still draft" do
      ...
      expect { EmbeddedContentFinderService.new.fetch_linked_friendly_ids(body, Edition::DEFAULT_LOCALE) }.to raise_error(CommandError)
    end
    ...
    it "errors when given a live content ID that is not available in the current locale" do
      ...
      expect { EmbeddedContentFinderService.new.fetch_linked_friendly_ids(body, "foo") }.to raise_error(CommandError)
    end
  end
end

RSpec.describe EmbeddedContentFinderService do
  describe ".fetch_linked_friendly_ids" do

...
RSpec.describe Events::S3Exporter do
  ...
  describe "#export" do
    ...
    context "when all of the items have been exported previously" do
      let!(:david_cameron_event) do
        FactoryGirl.create(:event, payload: nil, created_at: david_cameron_appointed)
      end
      let!(:gordon_brown_event) do
        FactoryGirl.create(:event, payload: nil, created_at: gordon_brown_appointed)
      end
      let!(:tony_blair_event) do
        FactoryGirl.create(:event, payload: nil, created_at: tony_blair_appointed)
      end
      let(:created_before) { theresa_may_appointed }

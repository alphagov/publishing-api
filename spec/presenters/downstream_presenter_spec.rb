...
RSpec.describe Presenters::DownstreamPresenter do
  def web_edition_for(edition)
    Queries::GetWebEditions.(edition.id).first
  end

  let(:state_fallback_order) { [] }
  let(:web_edition) { web_edition_for(edition) }
  let(:change_history) { { note: "Note", public_timestamp: 1.day.ago.to_s } }
  let(:details) { { body: "<p>Text</p>\n", change_history: [change_history], } }

  subject(:result) { described_class.present(web_edition, state_fallback_order: state_fallback_order) }
  ...
  describe "V2" do
    ...
    context "for a content item with dependencies" do
      ...
      it "expands the links for the content item" do
        result = described_class.present(web_edition_for(a), state_fallback_order: [:draft])

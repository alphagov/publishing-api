...
RSpec.describe Presenters::DownstreamPresenter do
  ...
  context "for a draft content item" do
    let!(:content_item) { FactoryGirl.create(:draft_content_item, :with_location, :with_translation, :with_semantic_version) }
    let!(:link_set) { FactoryGirl.create(:link_set, content_id: content_item.content_id) }
  ...
  describe "conditional attributes" do
    ...
    let!(:content_item) { FactoryGirl.create(:gone_draft_content_item, :with_location, :with_translation, :with_semantic_version, public_updated_at: nil) }

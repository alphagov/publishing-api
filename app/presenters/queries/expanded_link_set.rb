    ...
    private
      ...
      def present_expanded_link(link_hash)
        link_hash.tap do |hash|
          ...
          if hash[:details]
            hash[:details] = Presenters::DetailsPresenter.new(hash[:details], nil, content_embed_presenter(hash[:content_id], hash[:locale]), locale: hash[:locale]).details ## Use the `parsed_details` here
          end
        end
      end

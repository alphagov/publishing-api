class EmbeddedContentFinderService
  ...
  def fetch_linked_content_ids(body, locale)
    content_references = body.is_a?(Array) ? find_content_references(body[0][:content]) : find_content_references(body)
...
private
  ...
  def check_all_references_exist(content_references, locale)
    found_editions = editions(content_references, locale)
  ...
  def editions(content_references, locale)
